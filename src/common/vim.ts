...
export const appendTextOnCursor = async (text: string): Promise<void> => {
  const cursor = await window.getCursorPosition();
  const pos: Position = {
    line: cursor.line + 1,
    character: 0,
  };
  await (await workspace.document).buffer.setLines(text.split('\n'), { start: pos.line, end: pos.line });
  await window.moveTo(pos);
  // (await workspace.document).applyEdits([{ newText: text + '\n', range: { start: pos, end: pos } }], false, true);
};
